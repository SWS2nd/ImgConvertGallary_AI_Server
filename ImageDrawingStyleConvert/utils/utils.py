...
def upload_tensor_img(bucket, tensor, img_nm, drawing_style, conditional_number):
    if conditional_number == 1:
        tensor = tensor[:, :, [2, 1, 0]]  # 세로, 가로, 채널(BGR을 RGB로)
        image = Image.fromarray(tensor)  # tensor는 numpy.ndarray 형식, image 화
    elif conditional_number == 2:
    ...
    # s3 에다가 업로드
    ImagedrawingstyleconvertConfig.s3.put_object(Bucket=bucket, Key=upload_filename, Body=buffer, ACL='public-read')
    # s3 에 올라간 파일의 링크를 리턴함
    # 하는 이유는 location에 따라서 s3 파일이 올라가는 url이 달라지기 때문.
    location = ImagedrawingstyleconvertConfig.s3.get_bucket_location(Bucket=bucket)['LocationConstraint']
